...
const Pr_Detai4 = ({match, location}) => {
    ...
    return (
        <>
            <Helmet title="The 4th UNESCO-UNITWIN SYMPOSIUM/pre-session/4" />
            {/* pc */}
            <PWrapper>
                <PInner>
                    <PTop title={p_detaildata.session4.title}/>
                    <PVideo youtube={p_detaildata.session4.youtube} height="675px"/>
                    <PContentsArea>
                        <PLogoArea>
                            <PLogoImg src={p_detaildata.session4.img} alt="logo"/>
                        </PLogoArea>
                        <PTextArea>
                            <PTextTop>
                                <PPhotobtnArea href={p_detaildata.session4.photo} target="_blank">
                                    <PPototext>{curlang.p_data.phototext}</PPototext>
                                    <PPotoimg src={photoimg} alt="아이콘"/>
                                </PPhotobtnArea>
                            </PTextTop>
                            <PDiscription>{p_detaildata.session4.discription}</PDiscription>
                        </PTextArea>
                    </PContentsArea>
                    <PListArea>
                        <PItemTitle>{curlang.p_data.pr_showcase.title}</PItemTitle>
                        <PSection>
                            {p_detaildata.session4.list.map(list => {
            ...
            <MWrapper>
                <MTopArea>
                    <MTop title={m_detaildata.session4.title}/>
                </MTopArea>
                <MVideoArea>
                    <MVideo youtube={m_detaildata.session4.youtube}/>
                </MVideoArea>
                <MTextArea>
                    <MLogoImg src={m_detaildata.session4.img} alt="logo" />
                    <MPhotobtnArea href={m_detaildata.session4.photo} target="_blank">
                        <MPototext>{curlang.p_data.phototext}</MPototext>
                        <MPotoimg src={photoimg} alt="아이콘"/>
                    </MPhotobtnArea>
                    <MDiscription>{m_detaildata.session4.discription}</MDiscription>
                </MTextArea>
                <MItemSection>
                    <MItemSubject>{curlang.p_data.pr_showcase.title}</MItemSubject>
                    {m_detaildata.session4.list.map(list => {
                        return <MItemInner>
                                    <MItemLogoArea>
...
export default withRouter(Pr_Detai4)